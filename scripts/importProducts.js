...
if (existsSync(envPath)) {
  ...
  console.warn(
    "⚠️  Archivo .env no encontrado. Asegúrate de crearlo con tus credenciales."
  );
}
...
// Datos de productos de FakeStore API
const productos = [
  {
    id: 1,
...
async function importProducts() {
  ...
  try {
    for (const producto of productos) {
      try {
        // Remover el campo 'id' ya que Firestore genera su propio ID
        // También removemos 'rating' ya que no lo usamos en la app
        const { id, rating, ...productoParaFirestore } = producto;

        const docRef = await addDoc(
          collection(db, "products"),
          productoParaFirestore
        );
        console.log(
          `✅ Producto "${producto.title.substring(
            0,
            50
          )}..." agregado con ID: ${docRef.id}`
        );
        successCount++;
      } catch (error) {
        console.error(
          `❌ Error agregando producto "${producto.title}":`,
          error.message
        );
        errorCount++;
      }
